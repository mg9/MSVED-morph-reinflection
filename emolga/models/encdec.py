...
class Encoder(Model):
    ...
    def build_encoder(self, source, context=None, return_embed=False, return_sequence=False):
        ...
        if not self.config['bidirectional']:
            ...
            X_out1 = self.backwardRNN(X, X_mask,  C=context, init_h=Init_h, return_sequence=return_sequence) # (nb_sample, dim) / (nb_samples, max_len, dim)
        ...
        X_mask  = T.cast(X_mask, dtype=theano.config.floatX)
        if return_embed:
...
class Decoder(Model):
    ...
    def __init__(self,
        ...
        self.dropword = config['word_dropout']
        if self.dropout > 0:
            logger.info('Use standard-dropout!!!!')
            self.D   = Dropout(rng=self.rng, p=self.dropout, name='{}_Dropout'.format(prefix))
        if self.dropword > 0:
            logger.info('Use word dropout at decoder!!!')
            self.DW  = DropoutWord(rng=self.rng, p=self.dropword, name='{}_Dropword'.format(prefix))
        ...
        if config['provide_init_h']:
            pass
        elif not config['bias_code']:
    ...
    def build_decoder(self, target, context=None,
                      provide_init_h =None, return_count=False,
                      train=True):
        ...
        if self.init_h:
            Init_h = provide_init_h # (batch_size, dec_hidden_dim)
        else:
            Init_h = self.Initializer(context)
        ...
        log_prob = T.sum(T.log(self._grab_prob(prob_dist, target)) * X_mask, axis=1) # (batch_size, 1)
    ...
    def build_sampler(self, provide_init_h=None):
        """
        ...
        prev_stat = T.matrix('prev_state', dtype=theano.config.floatX)
    ...
    def get_sample(self, context=None, provide_init_state=None, k=1, maxlen=30, stochastic=True, argmax=False, fixlen=False):
        # beam size
        if k > 1:
        ...
        for ii in range(maxlen):
            ...
            if stochastic:
                ...
                trans_index = np.zeros(ranks_flat.shape, dtype='int64')
                temp = ranks_flat / voc_size
                for i in range(0, len(ranks_flat)):
                    trans_index[i] = int(temp[i])
...
class DecoderAttCxt(Decoder):
    ...
    def __init__(self,
                 config, rng, prefix='dec',
                 mode='RNN', embed=None,
                 copynet=False, identity=False, init_h=False):
        super(DecoderAttCxt, self).__init__(
                config, rng, prefix,
                 mode, embed, init_h=init_h)
    ...
    def build_decoder(self,
                      target,
                      context, c_mask, fixed_context,
                      provide_init_h=None,
                      return_count=False,
                      train=True): # In my case, fixed_context is lemma
        """
        ...
        def _recurrence(x, x_mask, prev_h, cc, cm, fc):
            # compute the attention and get the context vector
            prob  = self.attention_reader(prev_h, cc, Smask=cm)
            c     = T.sum(cc * prob[:, :, None], axis=1)
            cxt   = T.concatenate([c, fc], axis=1)
            x_out = self.RNN(x, mask=x_mask, C=cxt, init_h=prev_h, one_step=True)
            return x_out, prob, cxt

        outputs, _ = theano.scan(
            _recurrence,
            sequences=[X, X_mask],
            outputs_info=[Init_h, None, None],
            non_sequences=[context, c_mask, fixed_context]
        )
        ...
        if self.dropout > 0:
            readout = self.D(readout, train=train)
        # TODO: ctx already includes the attention vector and the fixed context
        if self.config['context_predict']:
            readin  += [Ctx]
        ...
        if return_count:
            ...
            return log_prob, log_ppl, Probs

    """
    Sample one step
    """

    def _step_sample(self, prev_word, prev_stat, context, c_mask, fixed_cxt):
        ...
        cxt = concatenate([cxt, fixed_cxt], axis=1)
    ...
    def build_sampler(self):
        ...
        fixed_cxt = T.matrix() # additional context for decoder
        ...
        next_prob, next_sample, next_stat \
            = self._step_sample(prev_word, prev_stat, context, c_mask, fixed_cxt)

        # next word probability
        logger.info('compile the function: sample_next')
        inputs = [prev_word, prev_stat, context, c_mask, fixed_cxt]
        outputs = [next_prob, next_sample, next_stat]
    ...
    Generate samples, either with stochastic sampling or beam-search!
    """
    def get_sample(self, context, c_mask, fixed_cxt, provide_init_state, k=1, maxlen=30, stochastic=True, argmax=False, fixlen=False):
        # beam size
        if k > 1:
            assert not stochastic, 'Beam search does not support stochastic sampling!!'
        ...
        for ii in range(maxlen):
            ...
            # TODO: CHECK HERE
            fcxt = np.tile(fixed_cxt, [live_k, 1])
            next_prob, next_word, next_state \
                = self.sample_next(next_word, next_state, ctx, cmk, fcxt)
...
class DecoderAtt(Decoder):
    ...
    def build_sampler(self):
        ...
        if not self.init_h:
            init_h = self.Initializer(context[:, 0, :])
            logger.info('compile the function: get_init_state')
            self.get_init_state = theano.function([context], init_h, name='get_init_state')
            logger.info('done.')
    ...
    Generate samples, either with stochastic sampling or beam-search!
    """
    def get_sample(self, context, c_mask, provide_init_h=None, k=1, maxlen=30, stochastic=True, argmax=False, fixlen=False):
        # beam size
        if k > 1:
            assert not stochastic, 'Beam search does not support stochastic sampling!!'
...
class TestDecoderAttCxt(Decoder):
    """
    Recurrent Neural Network-based Decoder
    with Attention Machenism
    """
    def __init__(self,
                 ...
                 mode='RNN', embed=None,
                 copynet=False, identity=False, init_h=False):
        super(TestDecoderAttCxt, self).__init__(
                config, rng, prefix,
                 mode, embed, init_h=init_h)
    ...
    def _step_sample(self, prev_word, prev_stat, context, c_mask, fixed_cxt):
        ...
        next_sample = self.rng.multinomial(pvals=next_prob).argmax(1)
        return next_prob, next_sample, next_stat, Probs
    ...
    def build_sampler(self):
        ...
        next_prob, next_sample, next_stat, att_probs \
            = self._step_sample(prev_word, prev_stat, context, c_mask, fixed_cxt)

        # next word probability
        logger.info('compile the function: sample_next')
        inputs = [prev_word, prev_stat, context, c_mask, fixed_cxt]
        outputs = [next_prob, next_sample, next_stat, att_probs]
    ...
    def get_sample(self, context, c_mask, fixed_cxt, provide_init_state, k=1, maxlen=30, stochastic=True, argmax=False, fixlen=False):
        ...
        for ii in range(maxlen):
            ...
            # TODO: CHECK HERE
            fcxt = np.tile(fixed_cxt, [live_k, 1])
            next_prob, next_word, next_state, att_probs \
                = self.sample_next(next_word, next_state, ctx, cmk, fcxt)
            ...
            if stochastic:
                ...
                # print "shape of att probs: ", att_probs.shape
                for idx, [ti, wi] in enumerate(zip(trans_index, word_index)):
                    new_hyp_samples.append(hyp_samples[ti] + [wi])
                    new_hyp_scores[idx] = copy.copy(costs[idx])
                    new_hyp_states.append(copy.copy(next_state[ti]))
                    # print ti
                    # print "att probs", att_probs[ti]
                    # print "hype att :", hyp_atts[ti]
                    new_hyp_atts.append(hyp_atts[ti] + [att_probs[ti]])
                    # print ti, " length of hyp samples: ", len(new_hyp_samples[-1])
                    # print ti, " length of hyp atts: ", len(new_hyp_atts[-1])
                # check the finished samples
                new_live_k = 0
                hyp_samples = []
                hyp_scores = []
                ...
                for idx in range(len(new_hyp_samples)):
                    if (new_hyp_states[idx][-1] == 0) and (not fixlen):
                        sample.append(new_hyp_samples[idx])
                        score.append(new_hyp_scores[idx])
                        attProbs.append(new_hyp_atts[idx])
                        # print idx, " length of dead samples: ", len(new_hyp_samples[-1])
                        # print idx, " length of dead atts: ", len(new_hyp_atts[-1])
                        dead_k += 1
                    else:
                        new_live_k += 1
                        hyp_samples.append(new_hyp_samples[idx])
        ...
        if not stochastic:
            ...
            if live_k > 0:
                for idx in range(live_k):
                    sample.append(hyp_samples[idx])
                    score.append(hyp_scores[idx])
                    attProbs.append(hyp_atts[idx])
        return sample, score, attProbs
